...
#include <kernel/sam/usb.h>
#include <kernel/sam/port.h>
#include <kernel/sam/powermgr.h>
...
extern unsigned usb_trim(void);
extern unsigned usb_transn(void);
extern unsigned usb_transp(void);
...
/* handler 'bottom-half' */
void
sam_usb_irq_bh(struct usb_dev *dev)
{
	u8 status = int_get();
	u32 epirq;
